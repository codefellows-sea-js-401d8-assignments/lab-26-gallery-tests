...
describe('test the list view option', function() {
  ...
  it('should list the images in the album', function() {
    ...
    this.scope.$digest();
    console.log(albumListTest.find('ul').children('li').eq(0).text());
    console.log(albumListTest.find('button')[0].text());
    albumListTest.find('button')[0].click();
    albumListTest.triggerHandler('input');
    console.log(albumListTest.find('a').text());
    expect(albumListTest.find('a').text()).toContain('Syndrome');
  });
});
